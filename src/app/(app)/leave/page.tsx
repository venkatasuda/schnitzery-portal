...
export default function LeavePage() {
  ...
  return (
    <div style={{ maxWidth: 560, margin: "0 auto" }}>
      ...
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>Request leave and track your requests.</p>
      ...
      {tab === "mine" && (
        <div>
          {loading ? <div style={{ color: "***REMOVED***9a8f8f", fontSize: 13, padding: 20, textAlign: "center" }}>Loading…</div>
          : requests.length === 0 ? <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 30 }}>No leave requests yet.</div>
          : requests.map((r) => (
            <div key={r.id} style={{ ...card, marginBottom: 8 }}>
              ...
              {r.decided_by && r.status !== "pending" && (
                <div style={{ fontSize: 11, color: "***REMOVED***6f6565", marginTop: 6 }}>Decided by {r.decided_by}</div>
              )}
            </div>
          ))}
        </div>
