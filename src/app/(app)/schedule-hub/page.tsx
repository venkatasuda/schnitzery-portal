            ...
            <div className="hub-tab-panel active">
              ...
              <div className="card">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                  ...
                  <Mini label={t("schedhub.submissions")} value={`${ov?.submissionCount ?? 0}/${ov?.staffCount ?? 0}`} color={(ov?.submissionCount ?? 0) > 0 ? "***REMOVED***58d68d" : "***REMOVED***ec7063"} />
                  <Mini label={t("schedhub.statusLabel")} value={ov?.rosterExists ? t("schedhub.built") : t("schedhub.open")} color={ov?.rosterExists ? "***REMOVED***58d68d" : "***REMOVED***e8a35a"} />
                </div>
              </div>

              <Link href="/approvals" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***1e8449,***REMOVED***27ae60)" }}>✅</div>
          ...
          {tab === "team" && (
            <div className="hub-tab-panel active">
              ...
              <Link href="/directory" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***2c3e50,***REMOVED***34495e)" }}>📇</div>
              ...
              <Link href="/notes" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***922b21,***REMOVED***c0392b)" }}>📝</div>
          ...
          {tab === "insights" && (
            <div className="hub-tab-panel active">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
                ...
                <Mini label={t("schedhub.submissions")} value={`${ov?.submissionCount ?? 0}/${ov?.staffCount ?? 0}`} color="***REMOVED***58d68d" />
              </div>
                ...
                <>
                  <div className="card">
                    ...
                    {(ov?.byTeamArr || []).map((tm: any) => (
                      <BarRow key={tm.team} label={teamLabel(tm.team)} count={tm.count} max={ov.totalShifts} color="***REMOVED***3498db" />
                    ))}
                  </div>
                  <div className="card">
                  ...
                  <div className="card">
                    ...
                    {(ov?.byDayArr || []).map((d: any) => (
                      <BarRow key={d.day} label={dayLabel(d.day)} count={d.count} max={Math.max(1, ...(ov.byDayArr || []).map((x: any) => x.count))} color="***REMOVED***d4a847" />
                    ))}
                  </div>
                </>
              )}
            ...
            <div className="hub-tab-panel active">
              {(ov?.conflicts || []).length === 0 ? (
                <div className="card" style={{ textAlign: "center", padding: 28 }}>
                  ...
                  <div style={{ color: "***REMOVED***58d68d", fontSize: 14, fontWeight: 600 }}>{t("schedhub.noConflicts")}</div>
                  ...
                  {ov.conflicts.map((c: any, i: number) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: i < ov.conflicts.length - 1 ? "1px solid rgba(128,128,128,0.12)" : "none" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "***REMOVED***e8a35a" }}>{c.type} — {c.detail}</div>
                      </div>
                    </div>
                  ))}
          ...
          {tab === "tools" && (
            <div className="hub-tab-panel active">
              ...
              <Link href="/schedule/compare" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***7d3c98,***REMOVED***af7ac5)" }}>📊</div>
              ...
              <Link href="/schedule/shift-times" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***1f6f54,***REMOVED***27ae60)" }}>⏱️</div>
              ...
              <Link href="/kiosks" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***34495e,***REMOVED***5d6d7e)" }}>🖥️</div>
              ...
              <Link href="/geofence" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***1e6091,***REMOVED***2980b9)" }}>📍</div>
              ...
              <Link href="/export" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***117a65,***REMOVED***16a085)" }}>📤</div>
              ...
              <Link href="/noshow" className="feature-card">
                <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***b9770e,***REMOVED***e67e22)" }}>📋</div>
