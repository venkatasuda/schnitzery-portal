...
export default function RosterPage() {
  ...
  if (denied) {
    return <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", maxWidth: 500, margin: "40px auto" }}>Only managers can edit the roster.</div>;
  }

  return (
    <div style={{ maxWidth: 760, margin: "0 auto" }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, fontFamily: "Georgia, serif", marginBottom: 2 }}>📋 Weekly Roster</h1>
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>Assign staff to shifts. {weekLabel}</p>
            ...
            <div key={day} style={{ ...card, marginBottom: 10 }}>
              ...
              {entries.map((e, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: TEAM_COLORS[e.team] || "***REMOVED***888" }} />
                  <div style={{ flex: 1, fontSize: 13 }}>
                    <b>{e.name}</b> <span style={{ color: "***REMOVED***9a8f8f" }}>· {e.team} · {e.shift}</span>
                  </div>
                  <button onClick={() => removeEntry(day, i)} style={{ background: "none", border: "none", color: "***REMOVED***ec7063", cursor: "pointer", fontSize: 16 }}>×</button>
                </div>
              ))}
              ...
              {entries.length === 0 && !isActive && (
                <div style={{ fontSize: 12, color: "***REMOVED***6f6565", marginTop: 6 }}>No one assigned.</div>
              )}
            </div>
          );
        })
...
const miniLbl: React.CSSProperties = { display: "block", fontSize: 10, color: "***REMOVED***9a8f8f", marginBottom: 4 };
const miniSelect: React.CSSProperties = { width: "100%", padding: "9px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "***REMOVED***fff", fontSize: 13, boxSizing: "border-box" };