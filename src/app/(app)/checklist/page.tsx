      ...
      {items.length === 0 ? (
        ...
        <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <button onClick={() => toggle(t)} disabled={busyId === t.id} style={{ width: 24, height: 24, borderRadius: 6, flexShrink: 0, cursor: "pointer",
            background: t.done ? "***REMOVED***27ae60" : "transparent", border: t.done ? "none" : "2px solid rgba(255,255,255,0.25)", color: "***REMOVED***fff", fontSize: 14 }}>
            {t.done ? <Icon e="✓" size={14} /> : ""}
          </button>
          <span style={{ flex: 1, fontSize: 14, color: t.done ? "***REMOVED***6f6565" : "***REMOVED***fff", textDecoration: t.done ? "line-through" : "none" }}>{t.task}</span>
          {canManage && <button onClick={() => del(t.id)} disabled={busyId === t.id} style={{ background: "none", border: "none", color: "***REMOVED***ec7063", cursor: "pointer", fontSize: 16 }}>×</button>}
        </div>
      ))}
    </div>
  );
