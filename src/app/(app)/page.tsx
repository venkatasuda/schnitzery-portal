  ...
  return (
    <>
      <Link href="/attendance" className="feature-card" style={{ background: clockedIn ? "linear-gradient(135deg,rgba(39,174,96,0.18),rgba(20,20,20,0.4))" : "linear-gradient(145deg,var(--dark2),var(--dark))" }}>
        <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***1a6b8a,***REMOVED***3498db)" }}>🕐</div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, margin: "4px 0 14px" }}>
        ...
        <Stat value={target != null ? `${pct}%` : "—"} label="Of Target" color={pct >= 80 ? "***REMOVED***58d68d" : "var(--white)"} />
      </div>

      <div className="section-label">Quick Actions</div>
      <Shortcut href="/schedule" icon="📅" grad="linear-gradient(135deg,***REMOVED***1e8449,***REMOVED***27ae60)" title="My Shifts" sub="Your weekly schedule & swaps" />
      <Shortcut href="/my-day" icon="🗓" grad="linear-gradient(135deg,***REMOVED***8b6914,***REMOVED***d4a847)" title="My Day" sub="Availability · time off · hours" />

      <div className="section-label">Workplace</div>
      <Shortcut href="/checklist" icon="✅" grad="linear-gradient(135deg,***REMOVED***1e8449,***REMOVED***27ae60)" title="Daily Checklist" sub="Opening & closing tasks" />
      <Shortcut href="/announcements" icon="📣" grad="linear-gradient(135deg,***REMOVED***922b21,***REMOVED***c0392b)" title="Announcements" sub="Team news & updates" />
      <Shortcut href="/directory" icon="📇" grad="linear-gradient(135deg,***REMOVED***2c3e50,***REMOVED***34495e)" title="Team Directory" sub="Find & contact colleagues" />
      <Shortcut href="/incidents" icon="🚨" grad="linear-gradient(135deg,***REMOVED***b9770e,***REMOVED***e67e22)" title="Report Incident" sub="Accidents, hazards & issues" />
    </>
  );
}
...
function ManagerDash({ ops, live, ot, sched }: {
  ...
  if (o.pendingApprovals > 0) alerts.push({ icon: "✅", href: "/approvals", text: `${o.pendingApprovals} approval${o.pendingApprovals === 1 ? "" : "s"} waiting`, color: "***REMOVED***e8a35a" });
  if (o.openIncidents > 0) alerts.push({ icon: "🚨", href: "/incidents", text: `${o.openIncidents} open incident${o.openIncidents === 1 ? "" : "s"}`, color: "***REMOVED***ec7063" });
  if (o.lowStock > 0) alerts.push({ icon: "📦", href: "/inventory", text: `${o.lowStock} low-stock item${o.lowStock === 1 ? "" : "s"}`, color: "***REMOVED***e8a35a" });
  if (missing > 0) alerts.push({ icon: "📋", href: "/noshow", text: `${missing} haven't submitted availability`, color: "***REMOVED***e8a35a" });
    ...
    <>
      ...
      {alerts.length === 0 ? (
        <div className="card" style={{ display: "flex", alignItems: "center", gap: 12, borderColor: "rgba(39,174,96,0.25)" }}>
          ...
          <div><div style={{ fontSize: 14, fontWeight: 600, color: "***REMOVED***58d68d" }}>All clear</div><div style={{ fontSize: 11, color: "var(--gray)" }}>Nothing needs your attention right now.</div></div>
        </div>
      ) : (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 10 }}>
        <Stat value={lv.workingNow} label="Working" color="***REMOVED***58d68d" />
        <Stat value={lv.completed} label="Done" />
        <Stat value={lv.late} label="Late" color={lv.late > 0 ? "***REMOVED***ec7063" : "var(--white)"} />
        <Stat value={fmtH(lv.totalMins)} label="Hours" color="var(--gold)" />
      </div>
      <Link href="/checklist" className="feature-card">
        <div className="feature-icon" style={{ background: checklistDone ? "linear-gradient(135deg,***REMOVED***1e8449,***REMOVED***27ae60)" : "linear-gradient(135deg,***REMOVED***b9770e,***REMOVED***e67e22)" }}>{checklistDone ? "✅" : "📋"}</div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
        ...
        <Stat value={fmtH(otd.totalOvertimeMins)} label="Overtime" color={otd.totalOvertimeMins > 0 ? "***REMOVED***e8a35a" : "var(--white)"} />
        <Stat value={otd.peopleOver} label="Over Contract" color={otd.peopleOver > 0 ? "***REMOVED***e8a35a" : "***REMOVED***58d68d"} />
      </div>
      ...
      <Shortcut href="/incidents" icon="🚨" grad="linear-gradient(135deg,***REMOVED***b9770e,***REMOVED***e67e22)" title="Report Incident" sub="Log accidents, hazards & issues" />
      <Shortcut href="/announcements" icon="📣" grad="linear-gradient(135deg,***REMOVED***922b21,***REMOVED***c0392b)" title="Post Announcement" sub="Send news to all staff" />
    </>
  );
}
  ...
  return (
    <>
      <Link href="/branches" className="feature-card" style={{ background: "linear-gradient(135deg,rgba(212,168,71,0.14),rgba(20,20,20,0.4))", borderColor: "rgba(212,168,71,0.3)" }}>
        <div className="feature-icon" style={{ background: "linear-gradient(135deg,***REMOVED***6b2fa0,***REMOVED***9b59b6)" }}>🏢</div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 14 }}>
        <Stat value={s.clockedIn} label="Working Now" color="***REMOVED***58d68d" />
        <Stat value={s.staffCount} label="Team Size" />
        <Stat value={s.pendingApprovals} label="Approvals" color={s.pendingApprovals > 0 ? "***REMOVED***e8a35a" : "var(--white)"} />
      </div>

      <div className="section-label">Manage</div>
      <Shortcut href="/roster" icon="📋" grad="linear-gradient(135deg,***REMOVED***1a6b8a,***REMOVED***3498db)" title="Weekly Roster" sub="Build the schedule" />
      <Shortcut href="/people-hub" icon="👥" grad="linear-gradient(135deg,***REMOVED***922b21,***REMOVED***c0392b)" title="People & Team" sub="Staff, directory & notes" />
      <Shortcut href="/inventory" icon="📦" grad="linear-gradient(135deg,***REMOVED***8b6914,***REMOVED***d4a847)" title="Inventory" sub="Stock counts & alerts" />
      <Shortcut href="/settings-hub" icon="⚙️" grad="linear-gradient(135deg,***REMOVED***555,***REMOVED***777)" title="Settings" sub="Config, payroll export & audit" />
    </>
  );
}
