...
export default function LeaveCalendarPage() {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>Approved upcoming time off across the team.</p>

      {loading ? (
        <div style={{ color: "***REMOVED***9a8f8f", padding: 30, textAlign: "center" }}>Loading…</div>
      ) : leave.length === 0 ? (
        <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 40 }}>☀️<br />No upcoming leave booked.</div>
      ) : (
        Object.entries(byMonth).map(([month, items]) => (
          <div key={month} style={{ marginBottom: 18 }}>
            <div style={{ fontSize: 12, letterSpacing: 1, color: "***REMOVED***9a8f8f", marginBottom: 8 }}>{month.toUpperCase()}</div>
            {items.map((l) => (
              <div key={l.id} style={{ ...card, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ fontSize: 12, color: "***REMOVED***d4a847", fontWeight: 600 }}>
