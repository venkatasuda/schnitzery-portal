...
export default function ChecklistStatusPage() {
  ...
  const dayColor = (done: number, total: number) =>
    total === 0 ? "***REMOVED***6f6565" : done === total ? "***REMOVED***58d68d" : done === 0 ? "***REMOVED***ec7063" : "***REMOVED***d4a847";

  const Section = ({ title, items }: { title: string; items: any[] }) => (
    <div style={{ ...card, marginBottom: 12 }}>
      <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>{title}</div>
      {items.length === 0 ? (
        <div style={{ fontSize: 13, color: "***REMOVED***6f6565" }}>{t("checklist.noTasks")}</div>
      ) : items.map((it) => (
        <div key={it.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <div style={{ width: 22, height: 22, borderRadius: 6, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
            background: it.done ? "***REMOVED***27ae60" : "transparent", border: it.done ? "none" : "2px solid rgba(236,112,99,0.55)" }}>
            {it.done && <Icon e="✓" size={13} color="***REMOVED***fff" />}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, color: "***REMOVED***fff" }}>{it.task}</div>
            <div style={{ fontSize: 11, color: it.done ? "***REMOVED***9a8f8f" : "***REMOVED***ec7063", marginTop: 2 }}>
              {it.done
          ...
          {it.input_kind === "number" && (
            <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "Georgia, serif", color: it.value ? "***REMOVED***d4a847" : "***REMOVED***6f6565", flexShrink: 0 }}>
              {it.value || "—"}
            </div>
          )}
        </div>
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 14 }}>{t("checklist.statusSub")}</p>

      <div style={{ ...card, display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        <label style={{ fontSize: 13, color: "***REMOVED***9a8f8f", flexShrink: 0 }}>{t("checklist.pickDate")}</label>
        <input type="date" value={date} max={berlinTodayStr()} onChange={(e) => setDate(e.target.value || berlinTodayStr())} style={{ ...input, flex: 1 }} />
        {!loading && summary.total > 0 && (
          <span style={{ fontSize: 14, fontWeight: 700, flexShrink: 0, color: summary.done === summary.total ? "***REMOVED***58d68d" : "***REMOVED***d4a847" }}>
      ...
      {history.length > 0 && (
        <div style={{ ...card, marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 11, color: "***REMOVED***9a8f8f", marginBottom: 8 }}>{t("checklist.historySub")}</div>
            ...
            {history.map((d) => {
              ...
              return (
                <button key={d.date} onClick={() => setDate(d.date)} style={{
                  ...
                  <span style={{ flex: 1, fontSize: 13, color: sel ? "***REMOVED***fff" : "***REMOVED***cfc5c5", fontWeight: sel ? 700 : 400 }}>{fmtDate(d.date)}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: c }}>
                    {d.done === d.total ? t("checklist.allDone") : `${d.done}/${d.total}`}
                  </span>
                  <span style={{ color: "***REMOVED***6f6565", fontSize: 14 }}>›</span>
                </button>
              );
            })}
          </div>
      ...
      {!loading && err && <div style={{ ...card, color: "***REMOVED***ec7063", fontSize: 13 }}>{err}</div>}
      {!loading && !err && tasks.length === 0 && <div style={{ ...card, color: "***REMOVED***9a8f8f", fontSize: 13 }}>{t("checklist.noTasksDate")}</div>}
...
const input: React.CSSProperties = { padding: "10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "***REMOVED***fff", fontSize: 14, boxSizing: "border-box" };