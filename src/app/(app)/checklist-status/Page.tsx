        ...
        <div key={it.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          {it.input_kind === "number" ? (
            <div style={{ width: 24, height: 24, borderRadius: 6, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
              background: it.done ? "***REMOVED***27ae60" : "transparent", border: it.done ? "none" : "2px solid rgba(255,255,255,0.25)", color: "***REMOVED***fff" }}>
            ...
            <button onClick={() => toggle(it)} disabled={busyId === it.id} style={{ width: 24, height: 24, borderRadius: 6, flexShrink: 0, cursor: "pointer",
              background: it.done ? "***REMOVED***27ae60" : "transparent", border: it.done ? "none" : "2px solid rgba(255,255,255,0.25)", color: "***REMOVED***fff", fontSize: 14 }}>
              {it.done ? <Icon e="✓" size={14} /> : ""}
            </button>
          )}
          <span style={{ flex: 1, fontSize: 14, color: it.done && it.input_kind !== "number" ? "***REMOVED***6f6565" : "***REMOVED***fff", textDecoration: it.done && it.input_kind !== "number" ? "line-through" : "none" }}>{it.task}</span>
          {it.input_kind === "number" && (
          ...
          {canManage && <button onClick={() => del(it.id)} disabled={busyId === it.id} style={{ background: "none", border: "none", color: "***REMOVED***ec7063", cursor: "pointer", fontSize: 16 }}>×</button>}
        </div>
      ))}
    </div>
  );
