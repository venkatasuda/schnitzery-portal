...
export default function NoShowPage() {
  ...
  if (denied) return <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", maxWidth: 500, margin: "40px auto" }}>Managers only.</div>;

  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, fontFamily: "Georgia, serif", marginBottom: 2 }}>⚠️ Attendance Checks</h1>
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>No-shows and missing availability.</p>
      ...
      {tab === "noshow" && (
        <div style={card}>
          ...
          <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f", marginBottom: 12 }}>Who was rostered but didn&apos;t clock in.</div>
          ...
          {nsChecked && (
            rosteredCount === 0 ? (
              <div style={{ color: "***REMOVED***9a8f8f", fontSize: 13, textAlign: "center", padding: 16 }}>No one was rostered that day.</div>
            ) : noShows.length === 0 ? (
              <div style={{ color: "***REMOVED***58d68d", fontSize: 13, textAlign: "center", padding: 16 }}>🎉 Everyone rostered clocked in!</div>
            ) : (
              <div>
                <div style={{ fontSize: 12, color: "***REMOVED***ec7063", marginBottom: 8 }}>{noShows.length} of {rosteredCount} rostered did not clock in:</div>
                {noShows.map((n, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    ...
                    <span style={{ fontSize: 12, color: "***REMOVED***9a8f8f" }}>{n.team} · {n.shift}</span>
                  </div>
                ))}
              </div>
            )
      ...
      {tab === "missing" && (
        <div style={card}>
          ...
          <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f", marginBottom: 14 }}>Who hasn&apos;t submitted their availability yet.</div>
          {missLoading ? (
            <div style={{ color: "***REMOVED***9a8f8f", fontSize: 13, textAlign: "center", padding: 16 }}>Loading…</div>
          ) : (
            <>
              {missing.length === 0 ? (
                <div style={{ color: "***REMOVED***58d68d", fontSize: 13, textAlign: "center", padding: 16 }}>🎉 Everyone has submitted!</div>
              ) : (
                <>
                  <div style={{ fontSize: 12, color: "***REMOVED***ec7063", marginBottom: 8 }}>{missing.length} not submitted:</div>
                  {missing.map((m) => (
              ...
              {submitted.length > 0 && (
                <div style={{ marginTop: 14, fontSize: 12, color: "***REMOVED***9a8f8f" }}>
...
const input: React.CSSProperties = { padding: "11px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "***REMOVED***fff", fontSize: 14, boxSizing: "border-box" };
const primaryBtn: React.CSSProperties = { padding: "12px", background: "***REMOVED***d4a847", color: "***REMOVED***1a0e0e", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer" };