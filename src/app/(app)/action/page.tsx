...
export default function ActionCenterPage() {
  ...
  const it = d?.items;
  const rows = it ? [
    { key: "approvals", icon: "✅", count: it.approvals.count, href: "/approvals", color: "***REMOVED***e8a35a",
    ...
    <div className="fade-up">
      ...
      {!loading && alerts.length > 0 && (
        <div className="card" style={{ marginBottom: 14, padding: 14 }}>
          <div className="section-label" style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}><Icon e="⚠️" size={14} color="***REMOVED***e8a35a" /> {t("act.alertsTitle")}</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {alerts.map((a) => {
              const color = a.severity === "high" ? "***REMOVED***ec7063" : "***REMOVED***e8a35a";
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: "center", padding: 36 }}>
          ...
          <div style={{ color: "***REMOVED***58d68d", fontSize: 16, fontWeight: 700 }}>{t("act.allClear")}</div>
        ...
        <>
          <div className="card" style={{ textAlign: "center", padding: 16, marginBottom: 14, background: "linear-gradient(135deg,rgba(232,163,90,0.12),rgba(20,20,20,0.4))" }}>
            <span style={{ fontSize: 30, fontWeight: 800, fontFamily: "var(--font-display)", color: "***REMOVED***e8a35a" }}>{d.total}</span>
