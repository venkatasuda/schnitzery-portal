...
export default function TimesheetPage() {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>Every shift, in detail.</p>
      ...
      : rows.length === 0 ? <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 30 }}>No shifts this month.</div>
      : (
        <>
          <div style={{ ...card, marginBottom: 10, display: "flex", justifyContent: "space-between", fontSize: 14 }}>
            <span style={{ color: "***REMOVED***9a8f8f" }}>Month total</span>
            <span style={{ color: "***REMOVED***d4a847", fontWeight: 700 }}>{fmtDur(totalMins)}</span>
          </div>
          {rows.map((r) => (
            <div key={r.id} style={{ ...card, marginBottom: 8, padding: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f", marginTop: 2 }}>
                    {fmtT(r.clock_in)} → {fmtT(r.clock_out)}
                    {r.status !== "complete" && <span style={{ color: "***REMOVED***e8a35a" }}> · {r.status}</span>}
                  </div>
                </div>
                <div style={{ fontSize: 14, fontWeight: 700, color: "***REMOVED***d4a847" }}>{r.status === "complete" ? fmtDur(r.duration_mins) : "—"}</div>
              </div>
            </div>
          ))}
        </>
