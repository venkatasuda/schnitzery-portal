...
export default function PayrollExportPage() {
  ...
  function downloadCsv() {
    if (rows.length === 0) { toast(t("exp2.nothingExport"), "error"); return; }
    const esc = (v: any) => { const s = String(v ?? ""); return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s; };
    const header = [t("exp2.csvCode"), t("exp2.csvName"), t("exp2.csvTeam"), t("exp2.csvShifts"), t("exp2.csvHours"), t("exp2.csvWage"), t("exp2.csvGross")];
    const lines = [header.join(",")];
    for (const r of rows) lines.push([r.code, r.name, r.team, r.shifts, r.hours, r.wage ?? "", r.gross ?? ""].map(esc).join(","));
    lines.push(["", t("exp2.csvTotal"), "", "", totals.hours, "", totals.gross].map(esc).join(","));
    const blob = new Blob(["\uFEFF" + lines.join("\r\n")], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `payroll-${month}.csv`; a.click();
    URL.revokeObjectURL(url);
    toast(t("exp2.csvDownloaded"), "success");
  }
  ...
  return (
    <div className="fade-up">
      <div className="page-title">📤 {t("profile.payrollExport")}</div>
      <div className="page-sub">{t("exp2.subtitle")}</div>

      <div className="card" style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <label style={{ fontSize: 12, color: "var(--gray)" }}>{t("exp2.month")}</label>
        <input
        ...
        <button onClick={downloadCsv} disabled={loading || rows.length === 0}
          style={{ padding: "9px 16px", background: rows.length === 0 ? "var(--dark3)" : "var(--gold)", color: rows.length === 0 ? "var(--gray)" : "***REMOVED***1a1a1a", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: rows.length === 0 ? "default" : "pointer" }}>
          {t("exp2.downloadCsv")}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: "center", padding: 30, color: "var(--gray)" }}>{t("exp2.noHours")}</div>
      ) : (
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.6fr 0.7fr 0.6fr 0.7fr 0.9fr", gap: 8, padding: "10px 14px", fontSize: 11, color: "var(--gray)", textTransform: "uppercase", letterSpacing: 0.5, borderBottom: "1px solid rgba(128,128,128,0.15)" }}>
            <span>{t("exp2.colName")}</span><span style={{ textAlign: "center" }}>{t("exp2.colShifts")}</span><span style={{ textAlign: "right" }}>{t("exp2.colHours")}</span><span style={{ textAlign: "right" }}>{t("exp2.colWage")}</span><span style={{ textAlign: "right" }}>{t("exp2.colGross")}</span>
          </div>
          {rows.map((r, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1.6fr 0.7fr 0.6fr 0.7fr 0.9fr", gap: 8, padding: "11px 14px", fontSize: 13, alignItems: "center", borderBottom: i < rows.length - 1 ? "1px solid rgba(128,128,128,0.08)" : "none" }}>
              ...
              <span style={{ textAlign: "right", color: r.wage == null ? "***REMOVED***e8a35a" : "var(--gray)" }}>{r.wage == null ? t("exp2.setWage") : eur(r.wage)}</span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.6fr 0.7fr 0.6fr 0.7fr 0.9fr", gap: 8, padding: "12px 14px", fontSize: 13, fontWeight: 700, background: "rgba(212,168,71,0.07)", borderTop: "1px solid rgba(128,128,128,0.15)" }}>
            <span style={{ color: "var(--white)" }}>{t("exp2.total")}</span><span /><span style={{ textAlign: "right", color: "var(--white)" }}>{totals.hours}h</span><span /><span style={{ textAlign: "right", color: "var(--gold)" }}>{eur(totals.gross)}</span>
          </div>
        </div>
      )}
      ...
      {rows.some((r) => r.wage == null) && (
        <div style={{ fontSize: 12, color: "var(--gray)", marginTop: 10 }}>
          {t("exp2.wageNoteA")} <Link href="/labor" style={{ color: "var(--gold)", textDecoration: "none" }}>{t("exp2.laborLink")}</Link> {t("exp2.wageNoteB")}
        </div>
      )}

      <Link href="/profile" style={{ display: "block", textAlign: "center", marginTop: 18, color: "var(--gold)", fontSize: 13, textDecoration: "none" }}>{t("exp2.backProfile")}</Link>
    </div>
  );
}