...
export default function ExportPage() {
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      ...
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>Hours per staff member for the month.</p>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, gap: 8, flexWrap: "wrap" }}>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => shiftMonth(-1)} style={navBtn}>‹ Prev</button>
          <span style={{ fontSize: 14, color: "***REMOVED***d4a847", fontWeight: 600, padding: "8px 4px" }}>{monthLabel}</span>
      ...
      {loading ? <div style={{ color: "***REMOVED***9a8f8f", padding: 30, textAlign: "center" }}>Loading…</div>
      : rows.length === 0 ? <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 30 }}>No completed shifts this month.</div>
      : (
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                  ...
                  <Td right><b style={{ color: "***REMOVED***d4a847" }}>{r.hours}h</b></Td>
              ...
              <tr style={{ borderTop: "2px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.02)" }}>
                ...
                <Td right><b style={{ color: "***REMOVED***d4a847" }}>{totalHours}h</b></Td><Td right></Td>
              </tr>
            </tbody>
          </table>
        </div>
...
function Th({ children, right }: any) {
  return <th style={{ padding: "12px 14px", textAlign: right ? "right" : "left", fontSize: 11, color: "***REMOVED***9a8f8f", fontWeight: 600, letterSpacing: 0.5 }}>{children}</th>;
}
function Td({ children, right }: any) {
  return <td style={{ padding: "11px 14px", textAlign: right ? "right" : "left", color: "***REMOVED***e8e0e0" }}>{children}</td>;
}
...
const primaryBtn: React.CSSProperties = { padding: "10px", background: "***REMOVED***d4a847", color: "***REMOVED***1a0e0e", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer" };