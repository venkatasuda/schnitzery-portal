        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 18 }}>
              <StatCell label="In" value={fmtTime(session?.clock_in)} color="***REMOVED***58d68d" />
              <StatCell label="Break" value={totalBreakMin > 0 ? `${totalBreakMin}m` : "—"} color="***REMOVED***e8a35a" />
              <StatCell label="Out" value={fmtTime(session?.clock_out)} color="***REMOVED***ec7063" />
            ...
            {!clockedIn ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <button onClick={() => doAction(clockIn, "✅ Clocked in!")} disabled={working} style={bigBtn("linear-gradient(135deg,***REMOVED***1e8449,***REMOVED***27ae60)", working)}>▶  Clock In Now</button>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {!onBreak ? (
                  <button onClick={() => doAction(startBreak, "☕ Break started")} disabled={working} style={bigBtn("linear-gradient(135deg,***REMOVED***b9770e,***REMOVED***e67e22)", working)}>☕  Start Break</button>
                ) : (
                  <button onClick={() => doAction(endBreak, "✅ Break ended")} disabled={working} style={bigBtn("linear-gradient(135deg,***REMOVED***117a65,***REMOVED***16a085)", working)}>✅  End Break</button>
                )}
                <button onClick={() => doAction(clockOut, "✅ Clocked out!")} disabled={working || onBreak} style={bigBtn("linear-gradient(135deg,***REMOVED***922b21,***REMOVED***c0392b)", working || onBreak)}>⏹  Clock Out</button>
                {!onBreak && (
                  <div style={{ display: "flex", gap: 8 }}>
        ...
        <div onClick={() => setShowCode(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} className="card" style={{ maxWidth: 360, width: "100%", padding: 24 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
              ...
              <button onClick={submitCode} disabled={working} style={{ ...bigBtn("linear-gradient(135deg,***REMOVED***1e8449,***REMOVED***27ae60)", working), flex: 1 }}>{working ? "…" : "Confirm"}</button>
            </div>
          </div>
        </div>
      )}
...
const filterBtn: React.CSSProperties = { padding: "9px 16px", background: "var(--gold)", color: "***REMOVED***1a0e0e", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" };
