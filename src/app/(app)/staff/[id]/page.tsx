...
function expiryStatus(expiry: string | null | undefined, t: Tf): { label: string; color: string } | null {
  ...
  if (days < 0) return { label: t("staffd.expired", { n: Math.abs(days) }), color: "***REMOVED***ec7063" };
  if (days <= 60) return { label: t("staffd.expiresIn", { n: days }), color: "***REMOVED***e8a35a" };
  return { label: t("staffd.validUntil", { date: new Date(expiry).toLocaleDateString([], { day: "2-digit", month: "short", year: "numeric" }) }), color: "***REMOVED***58d68d" };
}
