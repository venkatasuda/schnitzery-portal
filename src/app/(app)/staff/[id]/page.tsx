...
function expiryStatus(expiry?: string | null): { label: string; color: string } | null {
  ...
  if (days < 0) return { label: `Expired ${Math.abs(days)}d ago`, color: "***REMOVED***ec7063" };
  if (days <= 60) return { label: `Expires in ${days}d`, color: "***REMOVED***e8a35a" };
  return { label: `Valid until ${new Date(expiry).toLocaleDateString([], { day: "2-digit", month: "short", year: "numeric" })}`, color: "***REMOVED***58d68d" };
}
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 4 }}>
        ...
        <Stat value={overPct != null ? `${overPct}%` : "—"} label="Of Contract" color={overPct != null && overPct > 100 ? "***REMOVED***e8a35a" : "var(--white)"} />
      </div>
        ...
        <>
          ...
          <div className="card" style={{ marginBottom: 4 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 12 }}>
              ...
              <Stat value={bal.used} label="Taken" color="***REMOVED***e8a35a" />
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              />
              <button onClick={saveAllowance} style={{ padding: "8px 14px", background: "var(--gold)", color: "***REMOVED***1a1a1a", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Save</button>
            </div>
          </div>
        </>
      )}
