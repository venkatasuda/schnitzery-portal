      ...
      {showAdd && (
        <div onClick={() => setShowAdd(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", display: "flex", alignItems: "flex-start", justifyContent: "center", zIndex: 100, padding: 20, overflowY: "auto" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ ...card, maxWidth: 460, width: "100%", marginTop: 30 }}>
            ...
            <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f", marginBottom: 14 }}>They&apos;ll log in with this email + temporary password.</div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowAdd(false)} style={{ ...primaryBtn, background: "transparent", border: "1px solid rgba(255,255,255,0.15)", color: "***REMOVED***fff" }}>Cancel</button>
        ...
        <div key={p.id} style={{ ...card, marginBottom: 8 }}>
          {editId !== p.id ? (
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 42, height: 42, borderRadius: "50%", background: TEAM_COLORS[p.team] || "***REMOVED***666", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 700, color: "***REMOVED***fff" }}>
                {(p.full_name || "?")[0].toUpperCase()}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 15, fontWeight: 600 }}>{p.full_name || "—"} {p.employee_code && <span style={{ fontSize: 11, color: "***REMOVED***9a8f8f" }}>· {p.employee_code}</span>}</div>
                <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f" }}>{p.team || "No team"} · {p.role} · {p.contract_type || "—"}</div>
              </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                <button onClick={() => setEditId(null)} style={{ ...primaryBtn, background: "transparent", border: "1px solid rgba(255,255,255,0.15)", color: "***REMOVED***fff" }}>Cancel</button>
