      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
            <Stat value={data.counts.breakCount} label="Break Issues" color={data.counts.breakCount > 0 ? "***REMOVED***ec7063" : "***REMOVED***58d68d"} />
            <Stat value={data.counts.restCount} label="Rest Issues" color={data.counts.restCount > 0 ? "***REMOVED***ec7063" : "***REMOVED***58d68d"} />
            <Stat value={data.counts.longCount} label="Over 10h" color={data.counts.longCount > 0 ? "***REMOVED***e8a35a" : "***REMOVED***58d68d"} />
          </div>

          {v.length === 0 ? (
            <div className="card" style={{ textAlign: "center", padding: 32 }}>
              <div style={{ fontSize: 30, marginBottom: 8 }}>✅</div>
              <div style={{ color: "***REMOVED***58d68d", fontSize: 15, fontWeight: 700 }}>All compliant</div>
                ...
                {v.map((item: any, i: number) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 8px", borderBottom: i < v.length - 1 ? "1px solid rgba(128,128,128,0.12)" : "none" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: item.severity === "high" ? "***REMOVED***ec7063" : "***REMOVED***e8a35a" }}>{item.type} — {item.detail}</div>
                    </div>
                  </div>
                ))}
              </div>
