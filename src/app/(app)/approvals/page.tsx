...
export default function ApprovalsPage() {
  ...
  if (denied) {
    return <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", maxWidth: 500, margin: "40px auto" }}>Only managers can view approvals.</div>;
  }
    ...
    <div style={{ maxWidth: 640, margin: "0 auto" }}>
      ...
      {!loading && (
        <>
            ...
            <>
              ...
              {leave.map((r) => (
                <div key={r.id} style={{ ...card, marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 13, color: "***REMOVED***9a8f8f", margin: "4px 0" }}>
            ...
            <>
              ...
              {swaps.map((s) => (
                <div key={s.id} style={{ ...card, marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 13, color: "***REMOVED***9a8f8f", margin: "4px 0" }}>
                    Gives <b style={{ color: "***REMOVED***fff" }}>{s.my_day}</b>, wants <b style={{ color: "***REMOVED***fff" }}>{s.their_day}</b>
                  </div>
          ...
          {total === 0 && (
            <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 40 }}>
...
function SectionTitle({ children }: any) {
  return <div style={{ fontSize: 12, letterSpacing: 1, color: "***REMOVED***9a8f8f", margin: "16px 0 8px" }}>{children}</div>;
}
function DecisionButtons({ busy, onApprove, onDeny }: any) {
  return (
    <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
      <button onClick={onApprove} disabled={busy} style={{ flex: 1, padding: "10px", background: busy ? "***REMOVED***555" : "***REMOVED***27ae60", color: "***REMOVED***fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: busy ? "default" : "pointer" }}>✓ Approve</button>
      <button onClick={onDeny} disabled={busy} style={{ flex: 1, padding: "10px", background: "transparent", color: "***REMOVED***ec7063", border: "1px solid rgba(231,76,60,0.4)", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: busy ? "default" : "pointer" }}>✕ Deny</button>
    </div>
  );
}
