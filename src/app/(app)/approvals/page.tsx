    ...
    <div className="fade-up">
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: "center", padding: 34 }}>
          ...
          <div style={{ color: "***REMOVED***58d68d", fontSize: 15, fontWeight: 700 }}>Nothing to approve</div>
          ...
          {shown.map((item) => {
            ...
            return (
              <div key={item.kind + item.id} className="card" style={{ display: "flex", alignItems: "center", gap: 12, padding: 14 }}>
                ...
                <button onClick={() => decide(item, true)} disabled={busy} style={{ padding: "8px 12px", background: "***REMOVED***1e6b3f", color: "***REMOVED***fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: busy ? "default" : "pointer" }}>✓</button>
                <button onClick={() => decide(item, false)} disabled={busy} style={{ padding: "8px 12px", background: "var(--dark3)", color: "***REMOVED***ec7063", border: "1px solid rgba(231,76,60,0.3)", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: busy ? "default" : "pointer" }}>✕</button>
              </div>
            );
          })}
        </div>
