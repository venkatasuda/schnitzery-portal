"use client";

import { useEffect, useState } from "react";
import { getBranchSettings, saveBranchSettings } from "@/lib/queries/admin";

export default function SettingsPage() {
  const [qrRequired, setQrRequired] = useState(false);
  const [gpsMode, setGpsMode] = useState("off");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [denied, setDenied] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);

  useEffect(() => {
    (async () => {
      const res = await getBranchSettings();
      if (!res.ok) { if (res.error?.includes("Managers")) setDenied(true); setLoading(false); return; }
      setQrRequired(!!res.settings.qr_required);
      setGpsMode(res.settings.gps_mode || "off");
      setLoading(false);
    })();
  }, []);

  async function save() {
    setSaving(true); setMsg(null);
    const res = await saveBranchSettings(qrRequired, gpsMode);
    setSaving(false);
    setMsg(res.ok ? "✅ Settings saved!" : res.error || "Failed.");
  }

  if (denied) return <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", maxWidth: 500, margin: "40px auto" }}>Managers only.</div>;

  return (
    <div style={{ maxWidth: 520, margin: "0 auto" }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, fontFamily: "Georgia, serif", marginBottom: 2 }}>⚙️ Branch Settings</h1>
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 18 }}>Configure how your branch works.</p>

      {loading ? (
        <div style={{ color: "***REMOVED***9a8f8f", padding: 30, textAlign: "center" }}>Loading…</div>
      ) : (
        <>
          <div style={{ ...card, marginBottom: 12 }}>
            <label style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 600 }}>Require QR/code to clock in</div>
                <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f", marginTop: 2 }}>Staff must scan or enter the code (anti-fraud)</div>
              </div>
              <input type="checkbox" checked={qrRequired} onChange={(e) => setQrRequired(e.target.checked)} style={{ width: 20, height: 20 }} />
            </label>
          </div>

          <div style={{ ...card, marginBottom: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>GPS location check</div>
            <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f", marginBottom: 10 }}>Verify staff are at the restaurant when clocking in.</div>
            <select value={gpsMode} onChange={(e) => setGpsMode(e.target.value)} style={input}>
              <option value="off">Off — no location check</option>
              <option value="warn">Warn — flag if far away</option>
              <option value="required">Required — block if not on-site</option>
            </select>
          </div>

          <button onClick={save} disabled={saving} style={primaryBtn}>{saving ? "Saving…" : "💾 Save Settings"}</button>
          {msg && <div style={{ marginTop: 12, fontSize: 13, color: "***REMOVED***d4a847", textAlign: "center" }}>{msg}</div>}

          <div style={{ ...card, marginTop: 16, fontSize: 12, color: "***REMOVED***6f6565" }}>
            Note: GPS enforcement on clock-in is wired into settings here; the location check itself activates with the geo feature in a later phase.
          </div>
        </>
      )}
    </div>
  );
}
const card: React.CSSProperties = { background: "***REMOVED***241414", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 18 };
const input: React.CSSProperties = { width: "100%", padding: "11px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "***REMOVED***fff", fontSize: 14, boxSizing: "border-box" };
const primaryBtn: React.CSSProperties = { width: "100%", padding: "14px", background: "***REMOVED***d4a847", color: "***REMOVED***1a0e0e", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer" };