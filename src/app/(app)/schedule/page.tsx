...
export default function SchedulePage() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto" }}>
      ...
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>{weekLabel || "Loading…"}</p>
      ...
      {tab === "my" && (
        <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 13, color: "***REMOVED***d4a847" }}>{weekOffset === 0 ? "This week" : weekOffset > 0 ? `+${weekOffset} week(s)` : `${weekOffset} week(s)`}</span>
            <button onClick={() => changeWeek(1)} style={navBtn}>Next ›</button>
          </div>

          {/* week strip */}
          <div style={{ ...card, marginBottom: 10 }}>
            <div style={{ fontSize: 11, letterSpacing: 1, color: "***REMOVED***9a8f8f", marginBottom: 10 }}>THIS WEEK</div>
              ...
              {DAYS.map((d) => {
                ...
                return (
                  <div key={d} style={{ flex: 1, textAlign: "center", padding: "8px 2px", borderRadius: 8, background: isToday ? "rgba(212,168,71,0.15)" : "transparent" }}>
                    <div style={{ fontSize: 12, color: isToday ? "***REMOVED***d4a847" : "***REMOVED***9a8f8f", fontWeight: isToday ? 700 : 400 }}>{d[0]}</div>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", margin: "6px auto 0", background: has ? "***REMOVED***27ae60" : "rgba(255,255,255,0.12)" }} />
                  </div>
                );
              })}
            </div>
            <div style={{ fontSize: 11, color: "***REMOVED***9a8f8f", marginTop: 8, textAlign: "center" }}>
          ...
          {/* shifts list */}
          {loading ? (
            <div style={{ color: "***REMOVED***9a8f8f", fontSize: 13, padding: 20, textAlign: "center" }}>Loading…</div>
          ) : shifts.length === 0 ? (
            <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 30 }}>
              🗓<br />No shifts assigned this week.
            </div>
          ) : (
            <div style={card}>
              <div style={{ fontSize: 11, letterSpacing: 1, color: "***REMOVED***9a8f8f", marginBottom: 12 }}>YOUR SHIFTS</div>
              {shifts.map((s, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: i < shifts.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f" }}>{s.team} · {s.shift}</div>
                  </div>
                  <div style={{ fontSize: 13, color: "***REMOVED***d4a847", fontWeight: 600 }}>{s.time || "—"}</div>
                </div>
              ))}
            </div>
          )}
      ...
      {tab === "request" && (
        <div style={card}>
          ...
          <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f", marginBottom: 16 }}>Both you and your colleague will be notified of the result.</div>
          ...
          {swapMsg && <div style={{ marginTop: 12, fontSize: 13, color: "***REMOVED***d4a847", textAlign: "center" }}>{swapMsg}</div>}
        </div>
      )}
      ...
      {tab === "mine" && (
        <div>
          {mySwaps.length === 0 ? (
            <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 30 }}>No swap requests yet.</div>
          ) : (
            mySwaps.map((sw) => (
              <div key={sw.id} style={{ ...card, marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ fontSize: 14 }}>Give <b>{sw.my_day}</b> → get <b>{sw.their_day}</b></div>
                  <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 12, background: sw.status === "approved" ? "rgba(39,174,96,0.15)" : sw.status === "denied" ? "rgba(231,76,60,0.15)" : "rgba(212,168,71,0.15)", color: sw.status === "approved" ? "***REMOVED***58d68d" : sw.status === "denied" ? "***REMOVED***ec7063" : "***REMOVED***d4a847" }}>
                    {sw.status}
                  </span>
                </div>
              </div>
...
function TabBtn({ active, onClick, children }: any) {
  return (
    <button onClick={onClick} style={{ flex: 1, padding: "9px", background: active ? "***REMOVED***d4a847" : "transparent", color: active ? "***REMOVED***1a0e0e" : "***REMOVED***9a8f8f", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      {children}
    </button>
  );
}
function Field({ label, children }: any) {
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: "block", fontSize: 12, color: "***REMOVED***9a8f8f", marginBottom: 6 }}>{label}</label>
      {children}
    </div>
  );
}
...
const select: React.CSSProperties = { width: "100%", padding: "11px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "***REMOVED***fff", fontSize: 14, boxSizing: "border-box" };
