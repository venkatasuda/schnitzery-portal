...
export default function ProfilePage() {
  ...
  if (loading) return <div style={{ color: "***REMOVED***9a8f8f", padding: 30, textAlign: "center" }}>Loading…</div>;
  if (!profile) return <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", maxWidth: 500, margin: "40px auto" }}>Could not load profile.</div>;
    ...
    <div style={{ maxWidth: 560, margin: "0 auto" }}>
      ...
      <div style={card}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: "***REMOVED***d4a847", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 700, color: "***REMOVED***1a0e0e" }}>
            {(profile.full_name || "?")[0].toUpperCase()}
          </div>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700 }}>{profile.full_name || "—"}</div>
            <div style={{ fontSize: 13, color: "***REMOVED***d4a847" }}>{roleLabel[profile.role] || profile.role} · {profile.team || "No team"}</div>
          </div>
        </div>
        ...
        {!editing ? (
          ...
          <>
            ...
            <div style={{ fontSize: 11, color: "***REMOVED***6f6565", marginBottom: 12 }}>Team, contract & role are set by your manager.</div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setEditing(false); load(); }} style={{ ...primaryBtn, background: "transparent", border: "1px solid rgba(255,255,255,0.15)", color: "***REMOVED***fff" }}>Cancel</button>
              <button onClick={save} disabled={saving} style={primaryBtn}>{saving ? "Saving…" : "Save"}</button>
            </div>
          </>
        )}
        {msg && <div style={{ marginTop: 12, fontSize: 13, color: "***REMOVED***d4a847", textAlign: "center" }}>{msg}</div>}
      </div>
    </div>
  );
}
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", fontSize: 13 }}>
      <span style={{ color: "***REMOVED***9a8f8f" }}>{label}</span>
      <span style={{ color: "***REMOVED***fff", textAlign: "right" }}>{value}</span>
    </div>
  );
}
...
const input: React.CSSProperties = { width: "100%", padding: "11px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "***REMOVED***fff", fontSize: 14, boxSizing: "border-box" };
const primaryBtn: React.CSSProperties = { flex: 1, width: "100%", padding: "13px", background: "***REMOVED***d4a847", color: "***REMOVED***1a0e0e", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: "pointer" };