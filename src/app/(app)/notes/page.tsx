...
export default function NotesPage() {
  ...
  async function submit() {
    if (!userId || !note.trim()) { setMsg("Pick a person and write a note."); return; }
    setPosting(true); setMsg(null);
    const res = await addNote(userId, note, isRec);
    setPosting(false);
    if (res.ok) { setMsg("✅ Saved."); setNote(""); setUserId(""); setIsRec(false); load(); }
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, fontFamily: "Georgia, serif", marginBottom: 2 }}>📝 Notes & Recognition</h1>
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>Track performance notes and recognise great work.</p>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        ...
        <Field label="Note"><textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2} style={{ ...input, resize: "vertical" }} placeholder="What happened?" /></Field>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "***REMOVED***9a8f8f", marginBottom: 12, cursor: "pointer" }}>
          <input type="checkbox" checked={isRec} onChange={(e) => setIsRec(e.target.checked)} /> 🌟 Mark as recognition (positive)
        </label>
        <button onClick={submit} disabled={posting} style={primaryBtn}>{posting ? "Saving…" : "Add Note"}</button>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 4 }}>
        <TabBtn active={tab === "all"} onClick={() => setTab("all")}>All Notes</TabBtn>
        <TabBtn active={tab === "recognition"} onClick={() => setTab("recognition")}>🌟 Recognition</TabBtn>
      </div>

      {loading ? <div style={{ color: "***REMOVED***9a8f8f", padding: 20, textAlign: "center" }}>Loading…</div>
      : shown.length === 0 ? <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 30 }}>No notes yet.</div>
      : shown.map((n) => (
        <div key={n.id} style={{ ...card, marginBottom: 8, borderColor: n.isRecognition ? "rgba(212,168,71,0.3)" : "rgba(255,255,255,0.08)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 14, fontWeight: 600 }}>{n.isRecognition && "🌟 "}{n.subject?.full_name || "Staff"}</span>
