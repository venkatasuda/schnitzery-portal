...
export default function IncidentsPage() {
  ...
  return (
    <div style={{ maxWidth: 620, margin: "0 auto" }}>
      ...
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>Report accidents, hazards & issues.</p>
      ...
      {loading ? (
        ...
        <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 40 }}>✅<br />No incidents reported.</div>
      ) : (
        list.map((inc) => (
          <div key={inc.id} style={{ ...card, marginBottom: 10, borderColor: inc.status === "open" ? "rgba(231,76,60,0.25)" : "rgba(255,255,255,0.08)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 10, background: (SEV_COLOR[inc.severity] || "***REMOVED***666") + "26", color: SEV_COLOR[inc.severity] || "***REMOVED***aaa" }}>{(inc.severity || "").toUpperCase()}</span>
                <span style={{ fontSize: 13, fontWeight: 600 }}>{inc.category}</span>
              </div>
              <span style={{ fontSize: 11, padding: "2px 10px", borderRadius: 10, background: inc.status === "open" ? "rgba(231,76,60,0.15)" : "rgba(39,174,96,0.15)", color: inc.status === "open" ? "***REMOVED***ec7063" : "***REMOVED***58d68d" }}>{inc.status}</span>
            </div>
            ...
            <div style={{ fontSize: 11, color: "***REMOVED***6f6565", marginTop: 8 }}>
              ...
              {inc.manager_note && <div style={{ color: "***REMOVED***58d68d", marginTop: 4 }}>✓ {inc.reviewed_by}: {inc.manager_note}</div>}
            </div>
            ...
            {canManage && inc.status === "open" && (
              resolveId === inc.id ? (
                <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
                  ...
                  <button onClick={() => setResolveId(null)} style={{ ...primaryBtn, width: "auto", padding: "0 12px", background: "transparent", border: "1px solid rgba(255,255,255,0.15)", color: "***REMOVED***fff" }}>✕</button>
                </div>
              ) : (
