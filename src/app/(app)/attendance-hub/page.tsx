      ...
      {tab === "live" && (
        <div className="hub-tab-panel active">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
            <Stat value={stats.workingNow} label="Working" color="***REMOVED***58d68d" />
            <Stat value={stats.completed} label="Completed" color="var(--white)" />
            <Stat value={stats.late} label="Late" color={stats.late > 0 ? "***REMOVED***ec7063" : "var(--white)"} />
                ...
                return (
                  <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 8px", borderBottom: i < filtered.length - 1 ? "1px solid rgba(128,128,128,0.12)" : "none" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "var(--white)" }}>{r.name}{r.late_mins > 0 && <span style={{ fontSize: 10, color: "***REMOVED***ec7063", marginLeft: 8 }}>⚠ {r.late_mins}m late</span>}</div>
              ...
              <div key={r.id} className="card" style={{ marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "var(--white)" }}>
                    {r.name}
                    {r.overtime && <span style={{ fontSize: 10, color: "***REMOVED***e8a35a", marginLeft: 8 }}>⏱ overtime</span>}
                    {r.late_mins > 0 && <span style={{ fontSize: 10, color: "***REMOVED***ec7063", marginLeft: 8 }}>⚠ {r.late_mins}m late</span>}
                  </div>
          ...
          {otLoading ? (
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
                <Stat value={otStats.peopleOver} label="Over Contract" color={otStats.peopleOver > 0 ? "***REMOVED***e8a35a" : "***REMOVED***58d68d"} />
                <Stat value={fmtH(otStats.totalOvertimeMins)} label="Total Overtime" color={otStats.totalOvertimeMins > 0 ? "***REMOVED***e8a35a" : "var(--white)"} />
              </div>
                    ...
                    return (
                      <div key={r.user_id} style={{ padding: "11px 0", borderBottom: i < otRows.length - 1 ? "1px solid rgba(128,128,128,0.12)" : "none" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          ...
                          <div style={{ fontSize: 13, fontWeight: 700, color: over ? "***REMOVED***e8a35a" : "***REMOVED***58d68d" }}>{over ? `+${fmtH(r.overtimeMins)}` : "within"}</div>
                        </div>
