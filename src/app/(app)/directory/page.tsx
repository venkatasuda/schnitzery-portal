...
export default function DirectoryPage() {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <p style={{ color: "***REMOVED***9a8f8f", fontSize: 13, marginBottom: 16 }}>Find and contact colleagues.</p>
      ...
      {loading ? (
        ...
        <div style={{ ...card, textAlign: "center", color: "***REMOVED***9a8f8f", padding: 30 }}>No colleagues found.</div>
      ) : (
        filtered.map((p) => (
          <div key={p.id} style={{ ...card, marginBottom: 8, display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 600 }}>{p.full_name || "—"}{p.status === "inactive" && <span style={{ fontSize: 11, color: "***REMOVED***9a8f8f" }}> (inactive)</span>}</div>
              <div style={{ fontSize: 12, color: "***REMOVED***9a8f8f" }}>{p.team || "No team"}</div>
            </div>
